...
if( !empty($_POST)) {
    ...
    if( !isset($_POST['pseudo']) || strlen($_POST['pseudo']) < 4 || strlen($_POST['pseudo']) > 20 ) {
        $contenu = '<div class="alert alert-danger">Le pseudo doit contenir entre 4 et 20 caractères.</div>'; // si indice pseudo inf à 4 caractère ou sup à 20 caractère on affiche ce message
    }// fin if !isset($_POST['pseudo']

    if( !isset($_POST['mdp']) || strlen($_POST['mdp']) < 4 || strlen($_POST['mdp']) > 20 ) {
        $contenu = '<div class="alert alert-danger">Le mot depasse doit contenir entre 4 et 20 caractères.</div>'; // si indice pseudo inf à 4 caractère ou sup à 20 caractère on affiche ce message
    ...
    if (!isset($_POST['code_postal']) || !preg_match( '#^[0-9]{5}$#', $_POST['code_postal']) ) {
        $contenu .='<div class="alert alert-danger">le code postal n\'est pas valide.</div>';// est ce que le code postal correspond à l'expression régulière : la "regex" regular 
    }//  if (!isset($_POST['code_postal'])


    if( !isset($_POST['ville']) || strlen($_POST['ville']) < 1 || strlen($_POST['ville']) > 20 ) {
        $contenu = '<div class="alert alert-danger">Vérifier le nom de votre ville ?.</div>'; // si indice adresse inf à 4 caractère ou sup à 20 caractère on affiche ce message
    }// fin if !isset($_POST['ville']


    if (empty($contenu)) {// si la variable est vide, c'est que tu n'a pas d'erreur
        $membre = executeRequete(" SELECT * FROM membre WHERE pseudo = :pseudo",
                            ...
                            if ($membre->rowCount() > 0) { // si la requête retourne des lignes c'est que le pseudo existe déjà
                                ...
                                $mdp = password_hash($_POST['mdp'], PASSWORD_DEFAULT);// cette fonction prédéfinie permet de cripter le mot de passe selon l'algorithme actuel "bcrypt".  Il faudra lors de la connexion comparer le hash de la BDD avec celui du mdp de l'intérieur
...
<html lang="fr">
  ...
  <body>
    ...
    <!-- main -->
          <div class="row">
            <div class="col-sm-12 col-md-8 mx-auto p-4 text-center border border-success">
            ...
            <div class="form-group mt-2">
                ...
                <input type="radio" name="civilite" value="m"<?php if (isset($_POST['civilite']) && $_POST['civilite'] =='f') echo 'checked';?>> Homme          
            </div>
            <div class="form-group mt-2">
